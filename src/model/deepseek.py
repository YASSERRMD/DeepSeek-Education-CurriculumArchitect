...
class DeepSeek(nn.Module):
    ...
    def _compute_multi_token_loss(self, logits: torch.Tensor, targets: torch.Tensor):
        ...
        # The multi_token_predictor uses hidden states from sequence positions
        # 1, 2, ..., num_tokens to make its predictions. The hidden state at
        # position `t` is used to predict the token at `t+1`. Therefore, we need
        # the ground truth targets from the same positions to calculate the loss.
        # NOTE: This assumes the input sequence length is always > num_tokens.
        targets_sliced = targets[:, 1 : num_tokens + 1]

        # Reshape for loss computation. The first dimension of both tensors must match.
        logits_flat = logits.view(-1, vocab_size)       # Shape: [batch_size * num_tokens, vocab_size]
        targets_flat = targets_sliced.reshape(-1)       # Shape: [batch_size * num_tokens]
